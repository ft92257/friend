...
;(function($){
        ...
        $.each(allSelectBox, function(i){
            ...
            if (allSelectBox[i] != bid) {
                if (!$("#" + allSelectBox[i])[0]) {
                    ...
                    $("#"+allSelectBox[i]).find("i.combox-icon").removeClass('combox-icon-open');
    ...
    var _onchange = function (event){
        ...
        var options = MOB.jsonEval('{'+event.data.$this.attr("name")+':"'+event.data.$this.val()+'"}');
        ...
        $.ajax({
            ...
            success: function(json){
                ...
                var $refCombox = $ref.parents("div.selectbox:first");
    ...
    $.extend($.fn, {
        comboxSelect: function(options){
            var op = $.extend({ selector: ">a,>i" }, options);
            ...
            return this.each(function(){
                ...
                $(op.selector, box).click(function(){
                    $(">i", box).addClass('combox-icon-open');
                    ...
                    if (options.is(":hidden")) {
                        ...
                        options.css({width:box.width(),top:top,left:box.offset().left-1}).show();
        ...
        combox:function(){
            ...
            return this.each(function(i){
                ...
                var select = '<div class="selectbox" style="width: '+width+'px;"><div id="combox_'+ cid +'" class="select"' + (ref?' ref="' + ref + '"' : '') + '>';
                select += '<a href="javascript:" class="'+$this.attr("class")+'" name="' + name +'" value="' + value + '">' + label +'</a><i class="combox-icon"></i></div></div>';
                var options = '<ul class="comboxop" style="width: '+width+'px;" id="op_combox_'+ cid +'">';
                $("option", $this).each(function(){
                    var option = $(this);
                    options +="<li><a class=\""+ (value==option[0].value?"cur":"") +"\" href=\"javascript:;\" value=\"" + option[0].value + "\">" + option[0].text + "</a></li>";
                });
                options +="</ul>";
