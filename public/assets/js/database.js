...
(function($){
	...
	$.fn.extend({
		lookup: function(){
			return this.each(function(){
				...
				$this.click(function(event){
					...
					var opts = MOB.jsonEval($this.data('opt')) || {};
					opts.id = "noid";
					opts.title = opts.title || $this.text();
					...
					if (!url.isFinishedTm()) {
						alertMsg.error({content:opts.content || MOB.msg("alertSelectMsg")});
						return false;
					}else{
						options.url = url;
					}
					
					$.pdialog.open(options);
					return false;
				});
			});
		},
		multLookup: function(){
			return this.each(function(){
				var $this = $(this), args={},options = {mask:true,
					width:820, height:400
				};
				$this.click(function(event){
					...
					$unitBox.find("[name='"+$this.attr("multLookup")+"']").filter(":checked").each(function(){
						var _args = MOB.jsonEval($(this).val());
		...
		comboxLookup:function(){
			return this.each(function(){
				...
				$this.click(function(event){
					...
					var $unitBox = $this.parents(".unitBox:first");
					var comOptions = $unitBox.find("[name='"+$this.attr("comboxLookup")+"']").find("option:selected");

					var _args = MOB.jsonEval(comOptions.data("val"));
					for (var key in _args) {
						var value = args[key] ? args[key]+"," : "";
						args[key] = value + _args[key];
					}
		...
		suggest: function(){
			...
			return this.each(function(){
				var $input = $(this).attr('autocomplete', 'off').keydown(function(event){
					if (event.keyCode == MOB.keyCode.ENTER && $(op.suggest$).is(':visible')) return false; //屏蔽回车提交
				});
				
				var suggestFields=$input.attr('suggestFields').split(",");
				var W = $input.width();
				var opts = MOB.jsonEval($input.data('opt')) || {};
				opts.id = "noid";
				...
				function _show(event){
					...
					$.ajax({
						global:false,
						type:'get', dataType:"json", url:url, cache: false,
						data: postData,
						success: function(response){
							...
							if ($lis.size() == 1 && event.keyCode != MOB.keyCode.BACKSPACE) {
								_select($lis.eq(0));
							} else if ($lis.size() == 0){
								var jsonStr = "";
								for (var i=0; i<suggestFields.length; i++){
								...
								$.bringBackSuggest(MOB.jsonEval(jsonStr));
							}
						},
						error: function(){
							$suggest.html('');
				...
				$input.click(_show).keyup(function(event){
					var $items = $(op.suggest$).find("li");
					switch(event.keyCode){
						case MOB.keyCode.ESC:
						case MOB.keyCode.TAB:
						case MOB.keyCode.SHIFT:
						case MOB.keyCode.HOME:
						case MOB.keyCode.END:
						case MOB.keyCode.LEFT:
						case MOB.keyCode.RIGHT:
							break;
						case MOB.keyCode.ENTER:
							_close();
							break;
						case MOB.keyCode.DOWN:
		...
		itemDetail: function(){
			...
			function tdHtml(field){
				...
				var attrFrag = '';
				if (field.fieldAttrs){
					var attrs = MOB.jsonEval(field.fieldAttrs);
		...
		selectedTodo: function(){
			...
			return this.each(function(){
				...
				$this.click(function(){
					...
					if (!ids) {
						alertMsg.error($this.attr("warn") || MOB.msg("alertSelectMsg"));
						return false;
					}
