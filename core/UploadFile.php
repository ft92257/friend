...
class UploadFile
{
    ...
    private function save($file)
    {
        ...
        //move_uploaded_file 修改为rename
        if (!rename($file['tmp_name'], $this->autoCharset($filename, 'utf-8', 'gbk'))) {
            $this->error = '文件上传保存错误！';

            return false;
        }
        chmod($filename, 0666);//可被其他用户读写
        if ($this->thumb && in_array(strtolower($file['extension']), array('gif', 'jpg', 'jpeg', 'bmp', 'png'))) {
            ...
            if (false !== $image) {
                ...
                for ($i = 0, $len = count($thumbWidth); $i < $len; $i++) {
                    ...
                    Image::thumb($filename, $thumbPath . $thumbname . '.' . $thumbExt, '', $thumbWidth[$i],
                        $thumbHeight[$i], true);
                }
